                 ...
                 double *likeli,unsigned long *seed,int iter)
{
  ...
  double S,Birth_rate,T=1.,full_likelihood,max;
  ...
  while (1) {
    ...
    death_rate = NULL;
    death_rate = calc_death_rate(list,num_node,partial_likelihood,full_likelihood,Birth_rate,parms->nprior);
	...
	for (k=0;k<100;k++) {
            for (j=0;j<2;j++)
                tmp[j]=exp(rnorm(parms->theta[j],parms->re_precision[j],seed));
...
/*    a = (0<(temp=(y-ts[i][0]*0.6931472/parms->md[1])))?1000:temp;
    printf("a = %lf\n",a);
    node->mean_contrib[i] = node->theta[0]*x*exp(a);*/
    if(x == 0) node->mean_contrib[i] = 0;
    else node->mean_contrib[i] = node->theta[0]*x*exp(y - ts[i][0]*0.6931472/parms->md[1]);
    
/*  if(parms->flag == 1) printf("mci = %lf\n",node->mean_contrib[i]);*/
